...
def analyze_negative_clinics_cumulative_fixed():
    """
    부정적 감정을 가진 병원들의 월별 누적 조회수를 분석합니다. (수정된 버전)
    """
    ...
    print("=== 부정적 감정 병원 누적 조회수 분석 (수정된 버전) ===")
...
def create_cumulative_charts(negative_clinics, output_dir):
    ...
    for i, (clinic, posts) in enumerate(top_clinics):
        ...
        for date in date_range:
            # 날짜 형식을 'YYYY.MM'으로 통일하여 비교
            month_key = date.strftime('%Y.%m') # 수정된 부분
    ...
    plt.tight_layout()
    output_chart1 = os.path.join(output_dir, 'negative_clinics_individual_cumulative_fixed.png')
    ...
    plt.tight_layout()
    output_chart2 = os.path.join(output_dir, 'negative_clinics_comparison_cumulative_fixed.png')
...
def save_negative_analysis(negative_clinics, output_dir):
    ...
    output_csv = os.path.join(output_dir, 'negative_clinics_analysis_fixed.csv')
    ...
    detailed_csv = os.path.join(output_dir, 'negative_clinics_detailed_posts_fixed.csv')
    detailed_df.to_csv(detailed_csv, index=False, encoding='utf-8-sig')
    print(f"부정 게시글 상세 정보 저장: {detailed_csv}")

if __name__ == "__main__":
    analyze_negative_clinics_cumulative_fixed() 