...
REPORT_OUTPUT_PATH_HTML = os.path.join(RESULT_PATH, 'naver_place_analysis_v0.07_report.html')
TARGET_CLINIC = '내이튼치과의원'
ANALYSIS_DATE = '2025-08-04'

class NaverPlaceAnalyzerV0_07:
    ...
    def _create_tier_analysis_chart(self, tier_summary):
        ...
        # 차트 저장
        chart_filename = 'tier_analysis_chart_v0.07.png'
    ...
    def _extract_tier_descriptions(self, content):
        ...
        if content:
            import re
            # ### Tier X 패턴으로 섹션 분리
            sections = re.split(r'###\s*(Tier\s*\d+)', content)
            
            for i in range(1, len(sections), 2):
                if i+1 < len(sections):
                    tier_name = sections[i].strip()
                    tier_content = sections[i+1].strip()
                    
                    # 다음 섹션 전까지의 내용만 추출
                    next_section_idx = tier_content.find('###')
                    if next_section_idx > 0:
                        tier_content = tier_content[:next_section_idx]
                    
                    # HTML로 변환
                    tier_content_html = self.markdown_converter.convert(tier_content)
                    
                    tier_html += f'''
                    <div class="tab-content" id="tab-{tier_name}">
                        <h4>{tier_name}: {default_descriptions.get(tier_name, ("", ""))[0]}</h4>
                        {tier_content_html}
                    </div>
                    '''
        
        # LLM 컨텐츠가 없으면 기본 설명 사용
        if not tier_html:
    ...
    def generate_html_report_v0_07(self, sections):
        print("\nHTML 보고서 생성 (v0.07)...")
        introduction_html = """
        <h2>1. 분석의 배경 및 목적</h2>
        ...
        if market_overview:
            ...
            tier_descriptions = self._extract_tier_descriptions(content)
        ...
        html_template = f"""
        <!DOCTYPE html><html><head><meta charset="UTF-8"><title>'{TARGET_QUERY}' 네이버 플레이스 순위 분석 보고서 v0.07</title>
        ...
        </head><body><h1>[V0.07] '{TARGET_QUERY}' 네이버 플레이스 경쟁력 분석 및 성장 전략 제안</h1>
        {introduction_html}
        {content_html}
        </body></html>
        """
...
def main():
    analyzer = NaverPlaceAnalyzerV0_07(data_dir=RAW_DATA_DIR, result_path=RESULT_PATH)
    ...
    analyzer.generate_html_report_v0_07(sections)
