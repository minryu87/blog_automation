...
REPORT_OUTPUT_PATH_HTML = os.path.join(RESULT_PATH, 'naver_place_analysis_v0.06_report.html')
TARGET_CLINIC = '내이튼치과의원'
ANALYSIS_DATE = '2025-08-04'

class NaverPlaceAnalyzerV0_06:
    ...
    def _create_tier_analysis_chart(self, tier_summary):
        ...
        # 차트 저장
        chart_filename = 'tier_analysis_chart_v0.06.png'
    ...
    def _generate_interactive_chart_html(self, chart_data):
        """인터랙티브 차트 HTML 생성"""
        import json
        ...
        <div class="tab-container">
            ...
            <div class="tab-content" id="tab-Tier 1">
                <h4>Tier 1: 절대 강자 그룹</h4>
                <p>상위 2.5% 이내의 최상위 업체들로, 압도적인 경쟁력을 보유하고 있습니다.</p>
            </div>
            <div class="tab-content" id="tab-Tier 2">
                <h4>Tier 2: 도전자 그룹</h4>
                <p>상위 2.5-7.5%의 업체들로, Tier 1 진입을 노리는 강력한 경쟁자들입니다.</p>
            </div>
            <div class="tab-content" id="tab-Tier 3">
                <h4>Tier 3: 중위권 그룹</h4>
                <p>상위 7.5-17.5%의 업체들로, 안정적인 위치를 확보하고 있습니다.</p>
            </div>
            <div class="tab-content" id="tab-Tier 4">
                <h4>Tier 4: 성장 가능성 그룹</h4>
                <p>중하위권 업체들로, 전략적 노력에 따라 순위 상승 가능성이 있습니다.</p>
            </div>
            <div class="tab-content" id="tab-Tier 5">
                <h4>Tier 5: 진입 단계 그룹</h4>
                <p>하위권 업체들로, 기본적인 경쟁력 확보가 필요한 단계입니다.</p>
            </div>
        </div>
            ...
            options: {
                    ...
                    tooltip: {
                        callbacks: {
                            title: function(context) {
                                ...
                                const daysAgo = 60 - item.parsed.x;
                            ...
                            label: function(context) {
                                ...
                                return [
                                    ...
                                    '순위: ' + item.y + '위',
                                    '방문자 리뷰: ' + item.visitor_reviews,
                                    '블로그 리뷰: ' + item.blog_reviews,
                                    'Tier: ' + item.tier
                                ];
                            }
                        }
                ...
                onClick: function(evt, elements) {
                    if (elements.length > 0) {
                        ...
                        const company = chartData[datasetIndex];
                        
                        // 선택된 업체 정보 표시
                        const infoDiv = document.getElementById('selectedInfo');
                        let infoHtml = '<h4>' + company.company_name + ' 상세 정보</h4>';
                        infoHtml += '<table class="table table-striped">';
                        infoHtml += '<tr><th>시점</th><th>순위</th><th>Tier</th><th>방문자 리뷰</th><th>블로그 리뷰</th></tr>';
                        
                        company.data.sort((a, b) => a.days_ago - b.days_ago).forEach(point => {
                            const timeLabel = point.days_ago === 0 ? '현재' : point.days_ago + '일 전';
                            infoHtml += '<tr>';
                            infoHtml += '<td>' + timeLabel + '</td>';
                            infoHtml += '<td>' + point.rank + '위</td>';
                            infoHtml += '<td>' + point.tier + '</td>';
                            infoHtml += '<td>' + point.visitor_reviews + '</td>';
                            infoHtml += '<td>' + point.blog_reviews + '</td>';
                            infoHtml += '</tr>';
                        });
    ...
    def generate_html_report_v0_06(self, sections):
        print("\nHTML 보고서 생성 (v0.06)...")
        introduction_html = """
        <h2>1. 분석의 배경 및 목적</h2>
        ...
        if market_overview:
            ...
            if interactive_data:
                content_html += self._generate_interactive_chart_html(interactive_data)
            
            # LLM 분석 내용 추가 (차트 이미지 제외)
        ...
        html_template = f"""
        <!DOCTYPE html><html><head><meta charset="UTF-8"><title>'{TARGET_QUERY}' 네이버 플레이스 순위 분석 보고서 v0.06</title>
        ...
        </head><body><h1>[V0.06] '{TARGET_QUERY}' 네이버 플레이스 경쟁력 분석 및 성장 전략 제안</h1>
        {introduction_html}
        {content_html}
        </body></html>
        """
...
def main():
    analyzer = NaverPlaceAnalyzerV0_06(data_dir=RAW_DATA_DIR, result_path=RESULT_PATH)
    ...
    analyzer.generate_html_report_v0_06(sections)
