...
class NaverPlaceAnalyzerV0_10:
    ...
    def _load_and_prepare_time_series_data(self):
        ...
        # v0.09 방식: 현재 리뷰 수를 기준으로 과거 리뷰 수 계산
    ...
    def create_and_analyze_tiers(self):
        ...
        # 30일 전 데이터도 로드 (이미 _load_and_prepare_time_series_data에서 계산된 값 사용)
        ...
        # 60일 전 리뷰 데이터를 가져오기 (이미 _load_and_prepare_time_series_data에서 계산됨)
        past_reviews_df = self.time_series_df[self.time_series_df['days_ago'] == 60][['company_name', 'visitor_reviews', 'blog_reviews']].rename(columns={
            'visitor_reviews': 'visitor_reviews_60d_ago',
            'blog_reviews': 'blog_reviews_60d_ago'
        })
        
        # final_df에 60일 전 데이터 병합
        final_df = pd.merge(final_df, past_reviews_df, on='company_name', how='left')
        
        # 숫자형 변환
        final_df[['visitor_reviews_60d_ago', 'blog_reviews_60d_ago']] = final_df[['visitor_reviews_60d_ago', 'blog_reviews_60d_ago']].apply(pd.to_numeric, errors='coerce').fillna(0)
        final_df['visitor_reviews'] = pd.to_numeric(final_df['visitor_reviews'], errors='coerce').fillna(0)
        final_df['blog_reviews'] = pd.to_numeric(final_df['blog_reviews'], errors='coerce').fillna(0)
        
        # 증가량 계산: 현재 - 60일전
        final_df['visitor_reviews_change'] = final_df['visitor_reviews'] - final_df['visitor_reviews_60d_ago']
        final_df['blog_reviews_change'] = final_df['blog_reviews'] - final_df['blog_reviews_60d_ago']
    ...
    def analyze_market_overview(self, df_with_tiers, final_df):
        ...
        # 요청된 컬럼 순서 변경 ('현재 순위'를 맨 앞으로)
        final_columns = ['현재 순위', '병원명', '현재 Tier', '60일 전 Tier', '60일 전 순위', '방문자 리뷰 수', '방문자 리뷰 수 증가량', '블로그 리뷰 수', '블로그 리뷰 수 증가량']
        ...
        display_df = display_df.astype({'현재 순위': 'int', '60일 전 순위': 'Int64', '방문자 리뷰 수 증가량': 'Int64', '블로그 리뷰 수 증가량': 'Int64'})
...
"""
    ...
    def _generate_dynamic_table_html(self, df):
        """요청사항이 반영된 동적 HTML 테이블 생성"""
        
        table_id = "full-data-table"
        
        # HTML 테이블 헤더 생성 (정렬 아이콘 추가)
        headers = ""
        for i, col in enumerate(df.columns):
            headers += f'<th>{col} <span class="sort-asc" onclick="sortTable({i}, \'{table_id}\', \'asc\')">▲</span> <span class="sort-desc" onclick="sortTable({i}, \'{table_id}\', \'desc\')">▼</span></th>'

        # HTML 테이블 바디 생성
        body_rows = ""
        for i, row in df.iterrows():
            style = 'style="display: none;"' if i >= 10 else ''
            highlight_class = "table-pink" if row['병원명'] == TARGET_CLINIC else ""
            body_rows += f'<tr class="{highlight_class}" {style}>'
        ...
        <button onclick="showAllRows('{table_id}')" id="load-more-btn">전부 보기</button>
        """
    ...
    def generate_html_report_v0_10(self, sections):
        ...
        if market_overview:
            ...
            full_data_df = market_overview.get('full_data_df')
            if full_data_df is not None and not full_data_df.empty:
                content_html += self._generate_dynamic_table_html(full_data_df)
        ...
        <style>
            ...
            th{{background-color:#3498db;color:white; font-size: 15px; position: relative;}}
            .sort-asc, .sort-desc {{
            ...
            .table-pink {{background-color: #ffe4e1 !important;}}
            #load-more-btn {{display: block; margin: 20px auto; padding: 10px 20px; font-size: 16px; cursor: pointer;}}
        </style>
        <script>
            ...
            function sortTable(n, tableId, dir) {{
                var table, rows, switching, i, x, y, shouldSwitch;
                table = document.getElementById(tableId);
                switching = true;

                // 모든 정렬 아이콘 비활성화
                const allIcons = table.querySelectorAll('.sort-asc, .sort-desc');
                allIcons.forEach(icon => icon.classList.remove('active'));
                
                // 현재 클릭한 아이콘 활성화
                const headers = table.getElementsByTagName('th');
                const currentIcon = headers[n].querySelector(dir === 'asc' ? '.sort-asc' : '.sort-desc');
                ...
                while (switching) {{
                    ...
                    for (i = 1; i < (rows.length - 1); i++) {{
                        ...
                        var xContentText = x.innerHTML.replace(/,/g, '');
                        var yContentText = y.innerHTML.replace(/,/g, '');

                        var xContent = isNaN(parseFloat(xContentText)) ? xContentText.toLowerCase() : parseFloat(xContentText);
                        var yContent = isNaN(parseFloat(yContentText)) ? yContentText.toLowerCase() : parseFloat(yContentText);
...
def main():
    analyzer = NaverPlaceAnalyzerV0_10(data_dir=RAW_DATA_DIR, result_path=RESULT_PATH)
    ...
    analyzer.generate_html_report_v0_10(sections)
