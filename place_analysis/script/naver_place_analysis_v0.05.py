...
REPORT_OUTPUT_PATH_HTML = os.path.join(RESULT_PATH, 'naver_place_analysis_v0.05_report.html')
TARGET_CLINIC = '내이튼치과의원'
ANALYSIS_DATE = '2025-08-04'

class NaverPlaceAnalyzerV0_05:
    ...
    def analyze_market_overview(self, df_with_tiers, final_df):
        ...
        # Tier별 시계열 차트 생성
        tier_time_series_charts = self._create_tier_time_series_charts(df_with_tiers)
        ...
        return {
            ...
            "tier_time_series_charts": tier_time_series_charts
        }
    ...
    def _create_tier_analysis_chart(self, tier_summary):
        ...
        # 차트 저장
        chart_filename = 'tier_analysis_chart_v0.05.png'
    ...
    def _create_tier_time_series_charts(self, df_with_tiers):
        """각 Tier별 시계열 변화 차트 생성"""
        print("Tier별 시계열 차트 생성 중...")
        ...
        time_labels = ['60일 전', '30일 전', '20일 전', '10일 전', '5일 전', '현재']
        ...
        for tier in sorted(time_series_with_tier['unified_tier'].dropna().unique()):
            # 해당 Tier의 업체들만 필터링
            tier_data = time_series_with_tier[time_series_with_tier['unified_tier'] == tier]
            
            # 각 시점별 평균 계산
            metrics_by_time = []
            for days_ago in time_points:
                time_data = tier_data[tier_data['days_ago'] == days_ago]
                if not time_data.empty:
                    # 순위 변동성 계산 (현재 시점 기준 ±5일)
                    if days_ago == 0:
                        volatility_window = tier_data[tier_data['days_ago'].isin([0, 5])]
                    else:
                        volatility_window = tier_data[tier_data['days_ago'].isin([days_ago-5, days_ago, days_ago+5])]
                    
                    volatility = volatility_window.groupby('company_name')['rank'].std().mean()
                    
                    # 숫자형으로 변환
                    time_data['rank'] = pd.to_numeric(time_data['rank'], errors='coerce').fillna(0)
                    time_data['visitor_reviews'] = pd.to_numeric(time_data['visitor_reviews'], errors='coerce').fillna(0)
                    time_data['blog_reviews'] = pd.to_numeric(time_data['blog_reviews'], errors='coerce').fillna(0)
                    
                    metrics = {
                        'days_ago': days_ago,
                        'avg_rank': time_data['rank'].mean(),
                        'avg_visitor_reviews': time_data['visitor_reviews'].mean(),
                        'avg_blog_reviews': time_data['blog_reviews'].mean(),
                        'avg_volatility': volatility if not pd.isna(volatility) else 0
                    }
            ...
            if metrics_by_time:
                ...
                # 데이터 준비
                metrics_df = pd.DataFrame(metrics_by_time).sort_values('days_ago', ascending=False)
                x_pos = range(len(metrics_df))
                
                # 1. 평균 순위
                ax1.plot(x_pos, metrics_df['avg_rank'], 'ro-', linewidth=2, markersize=8)
                ax1.set_ylim(0, 200)
                ax1.set_xlabel('시점', fontsize=11)
                ax1.set_ylabel('평균 순위', fontsize=11)
                ax1.set_title(f'{tier} - 평균 순위 변화', fontsize=13, fontweight='bold')
                ax1.set_xticks(x_pos)
                ax1.set_xticklabels([time_labels[i] for i in range(len(metrics_df))], rotation=45)
                ax1.grid(True, alpha=0.3)
                
                # 2. 평균 방문자 리뷰
                ax2.plot(x_pos, metrics_df['avg_visitor_reviews'], 'bo-', linewidth=2, markersize=8)
                ax2.set_xlabel('시점', fontsize=11)
                ax2.set_ylabel('평균 방문자 리뷰 수', fontsize=11)
                ax2.set_title(f'{tier} - 평균 방문자 리뷰 변화', fontsize=13, fontweight='bold')
                ax2.set_xticks(x_pos)
                ax2.set_xticklabels([time_labels[i] for i in range(len(metrics_df))], rotation=45)
                ax2.grid(True, alpha=0.3)
                
                # 3. 평균 블로그 리뷰
                ax3.plot(x_pos, metrics_df['avg_blog_reviews'], 'go-', linewidth=2, markersize=8)
                ax3.set_xlabel('시점', fontsize=11)
                ax3.set_ylabel('평균 블로그 리뷰 수', fontsize=11)
                ax3.set_title(f'{tier} - 평균 블로그 리뷰 변화', fontsize=13, fontweight='bold')
                ax3.set_xticks(x_pos)
                ax3.set_xticklabels([time_labels[i] for i in range(len(metrics_df))], rotation=45)
                ax3.grid(True, alpha=0.3)
                
                # 4. 평균 순위 변동성
                ax4.plot(x_pos, metrics_df['avg_volatility'], 'mo-', linewidth=2, markersize=8)
                ax4.set_xlabel('시점', fontsize=11)
                ax4.set_ylabel('평균 순위 변동성', fontsize=11)
                ax4.set_title(f'{tier} - 평균 순위 변동성 변화', fontsize=13, fontweight='bold')
                ax4.set_xticks(x_pos)
                ax4.set_xticklabels([time_labels[i] for i in range(len(metrics_df))], rotation=45)
                ax4.grid(True, alpha=0.3)
                
                plt.suptitle(f'{tier} 시계열 분석 (기준일: {ANALYSIS_DATE})', fontsize=16, fontweight='bold')
                plt.tight_layout()
                
                # 차트 저장
                chart_filename = f'tier_{tier.replace(" ", "_")}_time_series_v0.05.png'
                chart_path = os.path.join(self.result_path, chart_filename)
                plt.savefig(chart_path, dpi=300, bbox_inches='tight')
                plt.close()
                
                tier_charts[tier] = chart_filename
                print(f"{tier} 시계열 차트 생성 완료: {chart_filename}")
        
        return tier_charts
    
    def generate_html_report_v0_05(self, sections):
        print("\nHTML 보고서 생성 (v0.05)...")
        introduction_html = """
        <h2>1. 분석의 배경 및 목적</h2>
        ...
        if market_overview:
            ...
            if 'chart_filename' in market_overview:
                ...
                content_html += f'</div>'
            # LLM 분석 내용을 파싱하여 각 Tier별로 차트 삽입
            content = market_overview.get('content', '')
            tier_charts = market_overview.get('tier_time_series_charts', {})
            
            # 컨텐츠를 Tier별로 분리하여 각 Tier 설명 다음에 차트 삽입
            if tier_charts and content:
                # Tier별로 내용 분리 및 차트 삽입
                content_parts = content.split('### ')
                processed_content = content_parts[0]  # 첫 부분 (Tier별 설명 전)
                
                for i, part in enumerate(content_parts[1:], 1):
                    processed_content += '### ' + part
                    
                    # 각 Tier에 해당하는 차트 찾기
                    for tier, chart_file in tier_charts.items():
                        if tier in part.split('\n')[0]:  # 첫 줄에 Tier 이름이 있는지 확인
                            # 해당 Tier 설명 끝에 차트 추가
                            processed_content += f'\n<div style="text-align: center; margin: 20px 0;">'
                            processed_content += f'<img src="{chart_file}" alt="{tier} 시계열 분석" style="max-width: 100%; height: auto;">'
                            processed_content += f'</div>\n'
                            break
                
                content_html += self.markdown_converter.convert(processed_content)
            else:
                content_html += self.markdown_converter.convert(content)
        ...
        html_template = f"""
        <!DOCTYPE html><html><head><meta charset="UTF-8"><title>'{TARGET_QUERY}' 네이버 플레이스 순위 분석 보고서 v0.05</title>
        ...
        </head><body><h1>[V0.05] '{TARGET_QUERY}' 네이버 플레이스 경쟁력 분석 및 성장 전략 제안</h1>
        {introduction_html}
        {content_html}
        </body></html>
        """
...
def main():
    analyzer = NaverPlaceAnalyzerV0_05(data_dir=RAW_DATA_DIR, result_path=RESULT_PATH)
    ...
    analyzer.generate_html_report_v0_05(sections)
