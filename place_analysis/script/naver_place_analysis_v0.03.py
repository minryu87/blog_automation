...
REPORT_OUTPUT_PATH_HTML = os.path.join(RESULT_PATH, 'naver_place_analysis_v0.03_report.html')
TARGET_CLINIC = '내이튼치과의원'
ANALYSIS_DATE = '2025-08-04'

class NaverPlaceAnalyzerV0_03:
    ...
    def analyze_market_overview(self, df_with_tiers, final_df):
        ...
        return {
            "title": f"동탄치과 플레이스 경쟁 현황 (기준 일자: {ANALYSIS_DATE})", 
    ...
    def analyze_target_clinic(self, df_with_tiers):
        ...
        ### 분석 요청사항
        '내이튼치과의원'의 현재 데이터와 목표 Tier와의 격차를 바탕으로, '리뷰부스터'와 '메디컨텐츠' 서비스 도입을 제안하는 구체적인 액션 플랜을 제시해주세요.
        ### 데이터
    ...
    def _create_tier_analysis_chart(self, tier_summary):
        ...
        ax2.invert_yaxis()  # 순위는 낮을수록 좋으므로 Y축 반전
        ...
        # 차트 저장
        chart_filename = 'tier_analysis_chart_v0.03.png'
    ...
    def generate_html_report_v0_03(self, sections):
        print("\nHTML 보고서 생성 (v0.03)...")
        introduction_html = """
        <h2>1. 분석의 배경 및 목적</h2>
        ...
        <p><i>(네이버 지도에서 '동탄치과'를 검색하였을 때의 순위를 기준으로 작성되었으며, 기준일은 2025년 8월 4일입니다.)</i></p>
        """
        content_html = ""
        ...
        html_template = f"""
        <!DOCTYPE html><html><head><meta charset="UTF-8"><title>'동탄치과' 네이버 플레이스 순위 분석 보고서 v0.03</title>
        ...
        </head><body><h1>[V0.03] '동탄치과' 네이버 플레이스 경쟁력 분석 및 성장 전략 제안</h1>
        {introduction_html}
        {content_html}
        </body></html>
        """
...
def main():
    analyzer = NaverPlaceAnalyzerV0_03(data_dir=RAW_DATA_DIR, result_path=RESULT_PATH)
    ...
    analyzer.generate_html_report_v0_03(sections)
